...
```typescript
// Using the standalone helper (works anywhere in request context)
...
// Or using the event builder method directly (more symmetric API)
const event = getWideEvent()
try {
  await processPayment(order)
} catch (error) {
...
```typescript
// ❌ This will NOT work - no context available
'use server'
export async function updateStatus(userId: string) {
  const event = requireWideEvent() // Throws: "called outside of a wide event context"
}

// ✅ This WILL work - withWideEvent creates/inherits context
'use server'
import { withWideEvent } from '@vestig/next/wide-events'

export const updateStatus = withWideEvent(
  async (userId: string, { event }) => {
    event.set('user', 'id', userId)
    // ...
  },
  { name: 'action.updateStatus' }
)
```
