...
```typescript
interface Transport {
  // Can return void (sync) or Promise<void> (async)
  log(entry: LogEntry): void | Promise<void>
}
```
...
```typescript
class BufferingTransport implements Transport {
  private buffer: LogEntry[] = []

  // Sync: Just adds to buffer, no waiting
...
```typescript
class ImmediateHttpTransport implements Transport {
  // Async: Each log is sent immediately
...
```typescript
// Internal logger behavior (simplified):
for (const transport of this.transports) {
  const result = transport.log(entry)
  // If async, the promise is NOT awaited by default
  // Logs are fire-and-forget for performance
}
```
...
await logger.flush()  // Waits for all transports to complete
```
